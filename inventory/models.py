...
class product(models.Model):
    ...
    name_fr          = models.CharField(max_length=125, blank=True, null=True, help_text="French translation of product name")
    name_es          = models.CharField(max_length=125, blank=True, null=True, help_text="Spanish translation of product name")
    ...
    def get_localized_name(self, lang=None):
        """
        Get localized product name with fallback to base name.
        
        Args:
            lang (str): Language code ('fr', 'es'). If None, uses current language.
            
        Returns:
            str: Localized name or fallback to base name if localized version is empty.
        """
        if lang is None:
            lang = get_language()
        
        # Extract language code (e.g., 'fr' from 'fr-CA')
        if lang:
            lang_code = lang.split('-')[0].lower()
            
            if lang_code == 'fr' and self.name_fr:
                return self.name_fr
            elif lang_code == 'es' and self.name_es:
                return self.name_es
...
class department(models.Model):
    ...
    department_name_fr = models.CharField(max_length=32, blank=True, null=True, help_text="French translation of department name")
    department_name_es = models.CharField(max_length=32, blank=True, null=True, help_text="Spanish translation of department name")
    ...
    def get_localized_name(self, lang=None):
        """
        Get localized department name with fallback to base name.
        ...
        if lang:
            ...
            if lang_code == 'fr' and self.department_name_fr:
                return self.department_name_fr
            elif lang_code == 'es' and self.department_name_es:
                return self.department_name_es
