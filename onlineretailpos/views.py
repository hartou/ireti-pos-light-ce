...
class EnterBarcode(forms.Form):
    barcode = forms.CharField(label=_("Barcode"), widget=forms.TextInput(attrs={'autofocus':"autofocus",' autocomplete':"off",'style':"width:100%"}),max_length = 32)
    qty = forms.IntegerField(label=_("Quantity"), widget=forms.TextInput(attrs={'style':"width:100%"}))
...
def register(request):
    ...
    # Compute cart totals from session (safe defaults)
    try:
    ...
    # Use existing template for the register screen
...
def dashboard_department(request):
    ...
    if not PLOTLY_AVAILABLE:
        context['error_message'] = "Charts are not available. Plotly library is not installed."
        context['form'] = form
...
def dashboard_sales(request):
    ...
    try:
        ...
        if df.empty or 'transaction_dt' not in df.columns:
            # Safe defaults when there is no data
            context['today_total_sales'] = 0.0
            context["add_info"] = {
                "Yesterday's Total Sales": 0.0,
                "Last 7 Days Avg Sales": 0.0,
                "WTD Total Sales": 0.0,
                "Last Week Total Sales": 0.0,
                "MTD Total Sales": 0.0,
                "YTD Total Sales": 0.0,
            }
            context['thirty_days_avg_sales'] = 0.0
            context['thirty_days_total_sales'] = 0.0

            # Empty/placeholder charts
            fig = px.bar(
                x=[today_date], y=[0], text_auto=True, barmode='group', template="plotly_white",
            ...
            context['thirty_day_sales_graph'] = po.plot(
                fig, auto_open=False, output_type='div', config={'displayModeBar': False}, include_plotlyjs=False
            )

            df_day_payment = pd.DataFrame({
                'payment_type': ['CASH', 'EBT', 'DEBIT/CREDIT'],
                'total_sale': [0, 0, 0]
            })
            fig2 = px.pie(
            ...
            context['day_payment_graph'] = po.plot(
                fig2, auto_open=False, output_type='div', config={'displayModeBar': False}, include_plotlyjs=False
            )
        else:
            ...
            context['today_total_sales'] = df_date.get(today_date, 0.0)
            context["add_info"] = {}
            context["add_info"]['Yesterday\'s Total Sales'] = df_date.get(today_date - timedelta(1), 0.0)
            last7_mask = df_date.index > today_date - timedelta(7)
            context["add_info"]['Last 7 Days Avg Sales'] = (df_date[last7_mask].sum() / 7) if last7_mask.any() else 0.0
            context['thirty_days_avg_sales'] = df_date[df_date.index > today_date - timedelta(30)].mean() if (df_date.index > today_date - timedelta(30)).any() else 0.0
            context['thirty_days_total_sales'] = df_date[df_date.index > today_date - timedelta(30)].sum() if (df_date.index > today_date - timedelta(30)).any() else 0.0

            weekly = df_date.resample('W').sum()
            monthly = df_date.resample('M').sum()
            yearly = df_date.resample('Y').sum()
            context["add_info"]['WTD Total Sales'] = weekly.iloc[-1] if len(weekly) else 0.0
            context["add_info"]['Last Week Total Sales'] = weekly.iloc[-2] if len(weekly) > 1 else 0.0
            context["add_info"]['MTD Total Sales'] = monthly.iloc[-1] if len(monthly) else 0.0
            context["add_info"]['YTD Total Sales'] = yearly.iloc[-1] if len(yearly) else 0.0
...
def offline_page(request):
    """
    PWA offline page - accessible without authentication for service worker.
    """
